...
plot!(orbitMED[:,2],orbitMED[:,3],label = "MED fwd",legend=:outerright)
plot!(orbitπMEQ[:,2],orbitπMEQ[:,3],label = "MEQ fwd",legend=:outerright)
# plot!(orbitMEQ[:,2],orbitMEQ[:,3],label = "MEQ fwd",legend=:outerright)
plot!(orbitMEr[:,2],orbitMEr[:,3],label = "MEr",legend=:outerright)
plot!(orbitπME[:,2],orbitπME[:,3],label = "π orbit",legend=:outerright)
plot!(orbitπMEr[:,2],orbitπMEr[:,3],label = "πr orbit",legend=:outerright)
plot!(orbitπMED[:,2],orbitπMED[:,3],label = "πD orbit",legend=:outerright)
# plot!(orbitCMEt[:,2],orbitCMEt[:,3],label = "tr orbit",legend=:outerright)
scatter!([CME.α[2]],[CME.α[3]],label = :false,series_annotations = ["     α"])
scatter!([CMEr.α[2]],[CMEr.α[3]],label = :false,series_annotations = ["     α̃"])
scatter!([orbit(1,CME)[2]],[orbit(1,CME)[3]],label = :false, series_annotations = ["       t=1"])
scatter!([orbit(1,CMEr)[2]],[orbit(1,CMEr)[3]],label = :false, series_annotations = ["       t̃=1"])
scatter!([orbit(0.1,CME)[2]],[orbit(0.1,CME)[3]],label = :false, series_annotations = ["        t=0.1"])
...
val = orbit(0.1,CME)*D./sum(orbit(0.1,CME)*D)
scatter!([val[2]],[val[3]],label = :false, series_annotations = ["\n\n              A(t)D t=0.1"])
