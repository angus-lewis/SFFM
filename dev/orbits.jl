...
τ = 0.5
scatter!([orbit(τ,CME)[4]],[orbit(τ,CME)[3]],label = :false, series_annotations = ["         t="*string(τ)])
# scatter!([orbit(1,πME)[4]],[orbit(1,πME)[3]],label = :false, series_annotations = ["        t=1"])
# scatter!([orbit(2.2885,πME)[4]],[orbit(2.2885,πME)[3]],label = :false, series_annotations = ["                                πexp(Qt)Q, t=2.2885"])
tstar = orbit_zero(CME,orbit(1,CMEt))
val = orbit(τ,CME)*exp(CME.Q*tstar)./sum(orbit(τ,CME)*exp(CME.Q*tstar))
scatter!([val[2]],[val[3]],label = :false, series_annotations = ["                         A(t)exp(Q) t=0.1"])
