...
Basis = "lagrange"
Δs = [1.6; 0.8; 0.4; 0.2]
NBasesRange = [1; 3; 5; 7; 9; 11; 13; 15]
...
for d = 1:length(Δs), n = 1:length(NBasesRange)
    ...
    NodesDG = collect(approxBounds[1, 1]:Δtemp:approxBounds[1, 2])
    NodesFV = collect(approxBounds[1, 1]:(Δtemp./nBases):approxBounds[1, 2])
    ...
    analyticX = SFFM.SFFMProbability(
        [pₓ[:]; 0; 0],
        Πₓ(Matrix(dgmesh.Nodes[2:end]')) - Πₓ(Matrix(dgmesh.Nodes[1:end-1]')),
        dgmesh.Nodes[1:end-1] + SFFM.Δ(dgmesh) / 2,
    )
...
let p = plot()
    types = [:solid,:dash,:dashdot,:dashdotdot,:solid,:dash,:dashdot,:dashdotdot]
